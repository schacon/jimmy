...
class HealthKitManager {
    ...
    private func fetchStepsData() async {
        ...
        for i in 0..<14 {
            guard let dayStart = calendar.date(byAdding: .day, value: -13 + i, to: calendar.startOfDay(for: endDate)),
    ...
    private func fetchDailySteps(for stepsType: HKQuantityType, predicate: NSPredicate, date: Date) async -> Double {
        return await withCheckedContinuation { continuation in
    ...
    private func calculateWeeklyAverages() {
        ...
        if !last7DaysWeight.isEmpty {
            weeklyAverageWeight = last7DaysWeight.map { $0.value }.reduce(0, +) / Double(last7DaysWeight.count)
        }
        
        // Calculate average steps for last 7 days
        let last7DaysSteps = stepsData.suffix(7)
        if !last7DaysSteps.isEmpty {
            weeklyAverageSteps = last7DaysSteps.map { $0.value }.reduce(0, +) / Double(last7DaysSteps.count)
        }
    }
} 